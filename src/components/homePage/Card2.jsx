...
function Card2(props) {
  return (
    <div
      className={`w-[275px] h-[400px] rounded-3xl bg-white shadow-xl overflow-hidden flex flex-col ${props.hid} ${props.blo}`}
    >
      {/* Top image */}
      ...
      <div className="flex-1 px-4 pt-3 pb-2 space-y-2">
        ...
        <div className="flex items-center gap-1 text-sm">
          ...
          <span className="text-gray-500 ml-1">{props.ratingText}</span>
        </div>

        {/* Price */}
        <div className="flex items-baseline gap-1 mt-1">
          <span className="text-2xl font-bold text-gray-900">${props.price}</span>
          <span className="text-sm text-gray-500">{props.per}</span>
        </div>
      </div>
